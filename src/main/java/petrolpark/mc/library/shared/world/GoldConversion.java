package petrolpark.mc.library.shared.world;
...
import petrolpark.mc.library.util.Conversion.ArmorItemConversion;
import petrolpark.mc.library.util.Conversion.ColorEntityConversion;
import petrolpark.mc.library.util.Conversion.ConversionResult;
import petrolpark.mc.library.util.Conversion.DyedBlockConversion;
import petrolpark.mc.library.util.Conversion.InventoryEntityConversion;
import petrolpark.mc.library.util.Conversion.ItemEntityConversion;
import petrolpark.mc.library.util.Conversion.ItemFrameItemConversion;
import petrolpark.mc.library.util.Conversion.RegisterConversionEvent;
import petrolpark.mc.library.util.Conversion.TieredItemConversion;
...
public class GoldConversion {
    ...
    public static final void onEntityTick(EntityTickEvent.Post event) {
        if (!(event.getEntity() instanceof LivingEntity livingEntity && livingEntity.hasEffect(SharedMobEffects.MIDAS_TOUCH.getDelegate()))) return;
        ...
        ItemHelper.modifyItems(livingEntity, stack -> GoldConversion.convertItemStackToGold(level, stack, player).value()); // Convert everything in the Inventory
        ...
        BlockPos.betweenClosedStream(box).forEach(pos -> { // Convert any touching Blocks
            ...
            if (!(converted.equals(block))) {
                ...
                if (level.isClientSide()) ParticleUtils.spawnParticlesOnBlockFaces(level, pos, SharedParticleTypes.GOLD.get(), UniformInt.of(3, 5));
            };
        });
    };
    ...
    public static final void onRegisterGoldItemConversions(RegisterGoldItemConversionEvent event) {
        ...
        register(event, "blocks", Conversion.convertBlockItem(GoldConversion::convertBlockToGold), 0);
    ...
    public static final void onRegisterGoldItemStackConversions(RegisterGoldItemStackConversionEvent event) {
        register(event, "contents", Conversion.convertItemStackContainerContents(GoldConversion::convertItemStackToGold), 10000);
        register(event, "trims", Conversion.convertItemStackComponentIfPresent(DataComponents.TRIM, GoldConversion::convertTrimToGold), 5500);
        register(event, "banner_patterns", Conversion.convertItemStackComponentIfPresent(DataComponents.BANNER_PATTERNS, GoldConversion::convertBannerToGold), 5000);
        register(event, "bundles", Conversion.convertItemStackComponentAndFinish(s -> s.getItem() == Items.BUNDLE, DataComponents.BASE_COLOR, GoldConversion::convertColorToGold), 2000);
        //TODO Ghast Harnesses
        register(event, "block_states", Conversion.convertItemStackBlockState(GoldConversion::convertBlockStateToGold), 1000);
        register(event, "items", Conversion.convertItemStackItem(GoldConversion::convertItemToGold), 0);
    };

    @SubscribeEvent
    public static final void onRegisterGoldBlockStateConversions(RegisterGoldBlockStateConversionEvent event) {
        register(event, "contents", Conversion.convertContainerContents(GoldConversion::convertItemStackToGold), 1000);
        register(event, "blocks", Conversion.convertBlockStateBlock(GoldConversion::convertBlockToGold), 0);
    };

    @SubscribeEvent
    public static final void onRegisterGoldEntityConversions(RegisterGoldEntityConversionEvent event) {
        register(event, "item_entities", new ItemEntityConversion(GoldConversion::convertItemStackToGold), 5000);
        register(event, "item_frames", new ItemFrameItemConversion(GoldConversion::convertItemStackToGold), 5000);
        register(event, "inventories", new InventoryEntityConversion(GoldConversion::convertItemStackToGold), 5000);
        
        register(event, "colored_entities",
            new ColorEntityConversion() {
                @Override
                public DyeColor convert(Entity entity, DyeColor color) {
                    return GoldConversion.convertColorToGold(entity.level(), color);
                };
            },
            2000
        );
