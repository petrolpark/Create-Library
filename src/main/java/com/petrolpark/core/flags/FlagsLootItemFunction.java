...
public record FlagsLootItemFunction(Holder<Flag> flagHolder, Action action) implements LootItemFunction {

    public static final MapCodec<FlagsLootItemFunction> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
        Flag.CODEC.fieldOf("flag").forGetter(FlagsLootItemFunction::flagHolder),
        Action.CODEC.optionalFieldOf("action", Action.ADD).forGetter(FlagsLootItemFunction::action)
    ).apply(instance, FlagsLootItemFunction::new));
    ...
    public LootItemFunctionType<? extends LootItemFunction> getType() {
        return PetrolparkLootItemFunctions.FLAGS.get();
    };
