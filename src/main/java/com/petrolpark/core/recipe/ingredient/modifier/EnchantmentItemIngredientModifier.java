...
public record EnchantmentItemIngredientModifier(EnchantmentPredicate enchantments) implements ItemIngredientModifier {

    public static final MapCodec<EnchantmentItemIngredientModifier> CODEC = CodecHelper.singleFieldMap(EnchantmentPredicate.CODEC, "enchantments", EnchantmentItemIngredientModifier::enchantments, EnchantmentItemIngredientModifier::new);
    public static final StreamCodec<RegistryFriendlyByteBuf, EnchantmentItemIngredientModifier> STREAM_CODEC = StreamCodec.composite(CodecHelper.ENCHANTMENT_PREDICATE_STREAM_CODEC, EnchantmentItemIngredientModifier::enchantments, EnchantmentItemIngredientModifier::new);
    ...
    public @Nonnull Optional<ItemStack> forbidLootItemFunction(LootItemFunction function, LootContext context, ItemStack stack) {
        ...
        return ItemIngredientModifier.super.forbidLootItemFunction(function, context, stack);
    };

    @Override
    public @Nullable Optional<MerchantOffer> forceTradeListing(ItemListing tradeListing, Entity trader, RandomSource random) {
        // TODO Auto-generated method stub
        return ItemIngredientModifier.super.forceTradeListing(tradeListing, trader, random);
    };

    @Override
    public @Nullable Optional<MerchantOffer> forbidTradeListing(ItemListing tradeListing, Entity trader, RandomSource random) {
        // TODO Auto-generated method stub
        return ItemIngredientModifier.super.forbidTradeListing(tradeListing, trader, random);
    };
    ...
    public INamedIngredientModifierType<ItemStack> getType() {
        return PetrolparkIngredientModifierTypes.ITEM_ENCHANTMENTS.get();
    };
    ...
    public static record Type(String translationKey) implements INamedIngredientModifierType<ItemStack> {
        ...
        @Override
        public Stream<EnchantmentItemIngredientModifier> streamApplicableModifiers(Level level, ItemStack stack) {
            if (stack.isEnchanted()) {
                return Stream.concat(
                    stack.getAllEnchantments(level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT)).entrySet().stream()
                        .flatMap(entry -> Stream.of(
                            new EnchantmentItemIngredientModifier(new EnchantmentPredicate(entry.getKey(), MinMaxBounds.Ints.ANY)), // Has each Enchantment
                            new EnchantmentItemIngredientModifier(new EnchantmentPredicate(entry.getKey(), MinMaxBounds.Ints.atLeast(entry.getIntValue()))) // Has each Enchantment at the required level
                        )),
                    Stream.of(new EnchantmentItemIngredientModifier(new EnchantmentPredicate(Optional.empty(), MinMaxBounds.Ints.ANY))) // Has any Enchantment
                );
