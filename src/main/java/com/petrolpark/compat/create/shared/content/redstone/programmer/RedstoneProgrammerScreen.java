...
import static com.petrolpark.compat.create.shared.content.redstone.programmer.RedstoneProgram.translate;
...
import com.petrolpark.compat.create.shared.content.redstone.programmer.RedstoneProgram.Channel;
import com.petrolpark.compat.create.shared.content.redstone.programmer.RedstoneProgram.PlayMode;
...
public class RedstoneProgrammerScreen extends AbstractSimiContainerScreen<RedstoneProgrammerMenu> {
    ...
    private final LerpedFloat horizontalScroll = LerpedFloat.linear().startWithValue(0d);
    private final LerpedFloat playhead = LerpedFloat.linear().startWithValue(0d);
    ...
    private final Map<PlayMode, IconButton> modeButtons;
    ...
    public RedstoneProgrammerScreen(RedstoneProgrammerMenu container, Inventory inv, Component title) {
        ...
        background = PetrolparkGuiTexture.REDSTONE_PROGRAMMER;
    ...
    protected void init() {
        ...
        for (PlayMode mode : PlayMode.values()) {
            IconButton button = new IconButton(leftPos + 27 + mode.ordinal() * 18, topPos + background.height - 24, mode.icon);
        ...
        ticksPerBeatScroller = new ScrollInput(leftPos + 6, topPos + background.height - 24, 18, 9)
            ...
            .titled(translate("ticks_per_beat"))
            .addHint(translate("ticks_per_beat.hint"))
            .withRange(PetrolparkConfigs.server().redstoneProgrammerMinTicksPerBeat.get(), 81);
        ...
        beatsPerLineScroller = new ScrollInput(leftPos + 156, topPos + background.height - 24, 18, 9)
            ...
            .titled(translate("beats_per_line"))
            .addHint(translate("beats_per_line.hint"))
            .withRange(0, 32);
        ...
        linesPerBarScroller = new ScrollInput(leftPos + 176, topPos + background.height - 24, 18, 9)
            ...
            .titled(translate("lines_per_bar"))
            .addHint(translate("lines_per_bar.hint"))
            .withRange(0, 32);
        ...
        followPlayheadButton.setToolTip(translate("follow_playhead"));
    ...
    public void containerTick() {
        ...
        // Sync to server
        if (shouldSend) {
            CatnipServices.NETWORK.sendToServer(new SetRedstoneProgramPacket(program));
            shouldSend = false;
        };
    };
                ...
                if (note >= 0 && note < program.getLength()) { // If we're scrolling on a valid note
                    ...
                    if (strength != 0) {
                        ...
                        for (int i = minSelectedNote; i <= maxSelectedNote; i++) {
                            channel.setStrength(i, Mth.clamp(strength + (int)deltaY, 1, 15));
                        };
                        shouldSend = true;
                        return true;
                    };
    ...
    protected void renderBg(@Nonnull GuiGraphics graphics, float partialTicks, int mouseX, int mouseY) {
        ...
        // Labels
        graphics.drawString(font, translate("playback"), 6, background.height - 34, AllGuiTextures.FONT_COLOR, false);
        graphics.drawString(font, translate("editor"), 156, background.height - 34, AllGuiTextures.FONT_COLOR, false);
        // Play time
        ...
        if (program.beatsPerLine > 0) {
            ...
            while (time < program.getLength()) {
                ...
                PetrolparkGuiTexture line = program.linesPerBar > 0 && time % (program.linesPerBar * program.beatsPerLine) == 0 ? PetrolparkGuiTexture.REDSTONE_PROGRAMMER_BARLINE : PetrolparkGuiTexture.REDSTONE_PROGRAMMER_LINE;
        ...
        for (Channel channel : program.getChannels()) {
            ...
            if (channelNo < program.getChannels().size() - 1) PetrolparkGuiTexture.REDSTONE_PROGRAMMER_MOVE_CHANNEL_DOWN.render(graphics, 16, 8);
        ...
        int programWidth = program.getLength() * noteWidth;
        if (program.getLength() > 1) PetrolparkGuiTexture.REDSTONE_PROGRAMMER_REMOVE_BAR.render(graphics, programWidth + 10, 80);
