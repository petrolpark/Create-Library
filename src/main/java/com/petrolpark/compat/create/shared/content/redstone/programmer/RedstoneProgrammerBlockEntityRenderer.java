...
public class RedstoneProgrammerBlockEntityRenderer extends SafeBlockEntityRenderer<RedstoneProgrammerBlockEntity> {
    ...
    protected void renderSafe(RedstoneProgrammerBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource bufferSource, int light, int overlay) {
        ...
        final SuperByteBuffer cylinder = CachedBuffers.partial(SharedPartialModels.REDSTONE_PROGRAMMER_CYLINDER, be.getBlockState())
            .center()
            .rotateYDegrees(AngleHelper.horizontalAngle(direction))
            .uncenter();
        final SuperByteBuffer needle = CachedBuffers.partial(SharedPartialModels.REDSTONE_PROGRAMMER_NEEDLE, be.getBlockState())
            .center()
            .rotateYDegrees(AngleHelper.horizontalAngle(direction))
            .uncenter();
        ...
        for (int i = 0; i < 6; i++) {
            ...
            CachedBuffers.partial(powered ? SharedPartialModels.REDSTONE_PROGRAMMER_TRANSMITTER_POWERED : SharedPartialModels.REDSTONE_PROGRAMMER_TRANSMITTER, be.getBlockState())
                .center()
                .rotateYDegrees(AngleHelper.horizontalAngle(direction))
